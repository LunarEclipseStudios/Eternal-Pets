...
$execute unless data storage $(target) $(path)[$(index)] run function lunareclipse.utils:forloop/cleanup with storage lunareclipse:utils temp_values.forloop
$execute unless data storage $(target) $(path)[$(index)] run return fail

# Store the info from the array somewhere that it is usable.
$data modify storage lunareclipse:utils temp_values.forloop.value set from storage $(target) $(path)[$(index)]

# Run the defined command.
$function $(command) with storage lunareclipse:utils temp_values.forloop
